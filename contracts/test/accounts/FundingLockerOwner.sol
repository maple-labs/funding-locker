...
contract FundingLockerOwner {
    ...
    function fundingLockerFactory_newLocker(address factory, address liquidityAsset) external returns (address) {
        return address(IFundingLocker(FundingLockerFactory(factory).newLocker(liquidityAsset)));
    }

    function try_fundingLockerFactory_newLocker(address factory, address liquidityAsset) external returns (bool ok) {
        (ok,) = factory.call(abi.encodeWithSignature("newLocker(address)", liquidityAsset));
    }
